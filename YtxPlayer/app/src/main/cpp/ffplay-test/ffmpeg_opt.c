                      ...
                      int (*open_file)(OptionsContext*, const char*))
{
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        if (ret < 0) {
            ALOGE("Error parsing options for %s file "
                   "%s.\n", inout, g->arg);
            return ret;
        }
...
int ffmpeg_parse_options(int argc, char **argv)
{
    ...
    if (ret < 0) {
        ALOGE("Error initializing complex filters.\n");
        goto fail;
    }
    ...
    if (ret < 0) {
        ALOGE("Error opening output files: ");
        goto fail;
    }
