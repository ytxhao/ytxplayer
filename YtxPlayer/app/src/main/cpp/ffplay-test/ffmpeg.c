...
#include "com_ytx_ican_ytxplayer_MainActivity.h"
#define LOG_NDEBUG 0
#define TAG "YTX-ffmpeg-JNI"
                                ...
                                AVSubtitleRect *r)
{
    ...
    if (r->type != SUBTITLE_BITMAP) {
        ALOGI( "sub2video: non-bitmap subtitle\n");
        return;
    }
    if (r->x < 0 || r->x + r->w > w || r->y < 0 || r->y + r->h > h) {
        ALOGI( "sub2video: rectangle (%d %d %d %d) overflowing %d %d\n",
            r->x, r->y, r->w, r->h, w, h
        );
        return;
    }
...
static void ffmpeg_cleanup(int ret)
{
    ...
    if (received_sigterm) {
        ALOGI( "Exiting normally, received signal %d.\n",
               (int) received_sigterm);
    } else if (ret && transcode_init_done) {
        ALOGI( "Conversion failed!\n");
    }
    term_exit();
    ffmpeg_exited = 1;
}
...
static void write_frame(AVFormatContext *s, AVPacket *pkt, OutputStream *ost)
{
    ...
    if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        if (ost->frame_rate.num && ost->is_cfr) {
            if (pkt->duration > 0)
                ALOGI( "Overriding packet duration by frame rate, this should not happen\n");
    ...
    if (!ost->st->codecpar->extradata && avctx->extradata) {
        ...
        if (!ost->st->codecpar->extradata) {
            ALOGE( "Could not allocate extradata buffer to copy parser data.\n");
            exit_program(1);
        }
    ...
    if (!(s->oformat->flags & AVFMT_NOTIMESTAMPS)) {
        if (pkt->dts != AV_NOPTS_VALUE &&
            ...
            pkt->dts > pkt->pts) {
            ALOGI("Invalid DTS: %"PRId64" PTS: %"PRId64" in output stream %d:%d, replacing by guess\n",
                   pkt->dts, pkt->pts,
     ...
     if(
      ...
      if (pkt->dts < max) {
        int loglevel = max - pkt->dts > 2 || avctx->codec_type == AVMEDIA_TYPE_VIDEO ? AV_LOG_WARNING : AV_LOG_DEBUG;
          ALOGI( "Non-monotonous DTS in output stream "
        ...
        if (exit_on_error) {
          ...
          ALOGI( "changing to %"PRId64". This may result "
                         ...
                         AVFrame *frame)
{
    ...
    if (avcodec_encode_audio2(enc, &pkt, frame, &got_packet) < 0) {
        ALOGE( "Audio encoding failed (avcodec_encode_audio2)\n");
        exit_program(1);
    }
                            ...
                            AVSubtitle *sub)
{
    ...
    if (sub->pts == AV_NOPTS_VALUE) {
        ALOGE( "Subtitle packets must have a pts\n");
        if (exit_on_error)
            exit_program(1);
        return;
    }
    ...
    if (!subtitle_out) {
        ...
        if (!subtitle_out) {
            ALOGE( "Failed to allocate subtitle_out\n");
            exit_program(1);
        }
    }
    ...
    for (i = 0; i < nb; i++) {
        ...
        if (subtitle_out_size < 0) {
            ALOGE( "Subtitle encoding failed\n");
            exit_program(1);
        }
                         ...
                         double sync_ipts)
{
    ...
    if (!next_picture) {
        ...
        if (delta0 < 0 &&
            ...
            if (delta0 < -0.6) {
                ALOGI( "Past duration %f too large\n", -delta0);
            } else
                ALOGI( "Clipping frame in rate conversion by %f\n", -delta0);
        ...
        case VSYNC_VSCFR:
            if (ost->frame_number == 0 && delta0 >= 0.5) {
                ALOGI( "Not duplicating %d initial frames\n", (int)lrintf(delta0));
                delta = duration;
                delta0 = 0;
    ...
    if (nb_frames > (nb0_frames && ost->last_dropped) + (nb_frames > nb0_frames)) {
        if (nb_frames > dts_error_threshold * 30) {
            ALOGE( "%d frame duplication too large, skipping\n", nb_frames - 1);
            nb_frames_drop++;
            return;
        }
  ...
  for (i = 0; i < nb_frames; i++) {
...
#endif
    {
        ...
        if (forced_keyframe) {
            ...
            ALOGI( "Forced keyframe at time %f\n", pts_time);
        }

        update_benchmark(NULL);
        if (debug_ts) {
            ALOGI( "encoder <- type:video "
                   "frame_pts:%s frame_pts_time:%s time_base:%d/%d\n",
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        while (1) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (debug_ts) {
                    ALOGI( "filter -> pts:%s pts_time:%s exact:%f time_base:%d/%d\n",
...
static void print_final_stats(int64_t total_size)
{
    ...
    ALOGI( "video:%1.0fkB audio:%1.0fkB subtitle:%1.0fkB other streams:%1.0fkB global headers:%1.0fkB muxing overhead: ",
           video_size / 1024.0,
           audio_size / 1024.0,
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        for (j = 0; j < f->nb_streams; j++) {
            ...
            ALOGI( "  Input stream #%d:%d (%s): ",
                   i, j, media_type_string(type));
            ALOGI( "%"PRIu64" packets read (%"PRIu64" bytes); ",
                   ist->nb_packets, ist->data_size);

            if (ist->decoding_needed) {
                ALOGI( "%"PRIu64" frames decoded",
                       ist->frames_decoded);
                if (type == AVMEDIA_TYPE_AUDIO)
                    ALOGI( " (%"PRIu64" samples)", ist->samples_decoded);
                ALOGI( "; ");
            }
        ...
        ALOGI( "  Total: %"PRIu64" packets (%"PRIu64" bytes) demuxed\n",
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        for (j = 0; j < of->ctx->nb_streams; j++) {
            ...
            ALOGI( "  Output stream #%d:%d (%s): ",
                   i, j, media_type_string(type));
            if (ost->encoding_needed) {
                ALOGI( "%"PRIu64" frames encoded",
                       ost->frames_encoded);
                if (type == AVMEDIA_TYPE_AUDIO)
                    ALOGI( " (%"PRIu64" samples)", ost->samples_encoded);
                ALOGI( "; ");
            }

            ALOGI( "%"PRIu64" packets muxed (%"PRIu64" bytes); ",
                   ost->packets_written, ost->data_size);

            ALOGI( "\n");
        }

        ALOGI( "  Total: %"PRIu64" packets (%"PRIu64" bytes) muxed\n",
               total_packets, total_size);
    }
    if(video_size + data_size + audio_size + subtitle_size + extra_size == 0){
        ALOGI( "Output file is empty, nothing was encoded ");
        if (pass1_used) {
            ALOGI( "\n");
        } else {
            ALOGI( "(check -ss / -t / -frames parameters if used)\n");
        }
    }
}
    ...
    if (  ost->st->codec->codec_id != AV_CODEC_ID_H264
       ...
       && ost->st->codec->codec_id != AV_CODEC_ID_VC1
        ...
        int ret = av_parser_change(ost->parser, ost->st->codec,
                             ...
                             pkt->flags & AV_PKT_FLAG_KEY);
        if (ret < 0) {
            ALOGE( "av_parser_change failed: %s\n",
                   av_err2str(ret));
            exit_program(1);
        }
        if (ret) {
...
int guess_input_channel_layout(InputStream *ist)
{
    ...
    if (!dec->channel_layout) {
        ...
        ALOGI( "Guessed Channel Layout for Input Stream "
...
static void check_decode_result(InputStream *ist, int *got_output, int ret)
{
    ...
    if (exit_on_error && *got_output && ist) {
        if (av_frame_get_decode_error_flags(ist->decoded_frame) || (ist->decoded_frame->flags & AV_FRAME_FLAG_CORRUPT)) {
            ALOGE( "%s: corrupt decoded frame in stream %d\n", input_files[ist->file_index]->ctx->filename, ist->st->index);
            exit_program(1);
        }
    }
}
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (resample_changed) {
        ...
        if (!guess_input_channel_layout(ist)) {
            ALOGE( "Unable to find default channel "
        ...
        for (i = 0; i < nb_filtergraphs; i++)
            if (ist_in_filtergraph(filtergraphs[i], ist)) {
                ...
                if (configure_filtergraph(fg) < 0) {
                    ALOGE("Error reinitializing filters!\n");
                    exit_program(1);
                }
            }
    }
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    if (*got_output && ret >= 0) {
        if (ist->dec_ctx->width  != decoded_frame->width ||
            ...
            ALOGI("Frame parameters mismatch context %d,%d,%d != %d,%d,%d\n",
                decoded_frame->width,
    ...
    if (debug_ts) {
        ALOGI("decoder -> ist_index:%d type:video "
...
static int process_input_packet(InputStream *ist, const AVPacket *pkt, int no_eof)
{
    ...
    handle_eof:
        ...
        if (avpkt.size && avpkt.size != pkt->size &&
            ...
            ALOGI( ist->showed_multi_packet_warning ? AV_LOG_VERBOSE : AV_LOG_WARNING,
        ...
        if (ret < 0) {
            ALOGE( "Error while decoding stream #%d:%d: %s\n",
    ...
    if (!pkt && ist->decoding_needed && !got_output && !no_eof) {
        ...
        if (ret < 0) {
            ALOGE( "Error marking filters as finished\n");
            exit_program(1);
        }
    }
...
static void print_sdp(void)
{
    ...
    if (!sdp_filename) {
        ...
        if (avio_open2(&sdp_pb, sdp_filename, AVIO_FLAG_WRITE, &int_cb, NULL) < 0) {
            ALOGE( "Failed to open sdp file '%s'\n", sdp_filename);
        } else {
        ...
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_DVB_SUBTITLE &&
           (ist->decoding_needed & DECODING_FOR_OST)) {
            ...
            if (ist->decoding_needed & DECODING_FOR_FILTER)
                ALOGI( "Warning using DVB subtitles for filtering and output at the same time is not fully supported, also see -compute_edt [0|1]\n");
        }
...
static int init_output_stream(OutputStream *ost, char *error, int error_len)
{
    ...
    if (ost->encoding_needed) {
        ...
        if (ost->enc_ctx->bit_rate && ost->enc_ctx->bit_rate < 1000)
            ALOGI( "The bitrate parameter is set too low."
                                    ...
                                    AVCodecContext *avctx)
{
    ...
    if (!pts) {
        ALOGE( "Could not allocate forced key frames array.\n");
        exit_program(1);
    }
...
static int transcode_init(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            case AVMEDIA_TYPE_AUDIO:
                if (audio_volume != 256) {
                    ALOGE( "-acodec copy and -vol are incompatible (frames are not decoded)\n");
                    exit_program(1);
                }
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (   av_q2d(enc_ctx->time_base) < 0.001 && video_sync_method != VSYNC_PASSTHROUGH
                   && (video_sync_method == VSYNC_CFR || video_sync_method == VSYNC_VSCFR || (video_sync_method == VSYNC_AUTO && !(oc->oformat->flags & AVFMT_VARIABLE_FPS)))){
                    ALOGI( "Frame rate very high for a muxer not efficiently supporting it.\n"
...
//         assert_avoptions(output_files[i]->opts);
 ...
 dump_format:
    ...
    ALOGI( "Stream mapping:\n");
    for (i = 0; i < nb_input_streams; i++) {
        ist = input_streams[i];

        for (j = 0; j < ist->nb_filters; j++) {
            if (ist->filters[j]->graph->graph_desc) {
                ALOGI( "  Stream #%d:%d (%s) -> %s",
                       ist->file_index, ist->st->index, ist->dec ? ist->dec->name : "?",
                       ist->filters[j]->name);
                if (nb_filtergraphs > 1)
                    ALOGI( " (graph %d)", ist->filters[j]->graph->index);
                ALOGI( "\n");
            }
        }
    }
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->filter && ost->filter->graph->graph_desc) {
            ...
            ALOGI( "  %s", ost->filter->name);
            if (nb_filtergraphs > 1)
                ALOGI( " (graph %d)", ost->filter->graph->index);
...
static OutputStream *choose_output(void)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->st->cur_dts == AV_NOPTS_VALUE)
            ALOGI("cur_dts is invalid (this is harmless if it occurs once at the start per stream)\n");
...
static int check_keyboard_interaction(int64_t cur_time)
{
    ...
    if (key == 'c' || key == 'C'){
        ...
        if (k > 0 &&
            ...
            ALOGI( "Processing command target:%s time:%f command:%s arg:%s",
...
static int init_input_threads(void)
{
    ...
    for (i = 0; i < nb_input_files; i++) {
        ...
        if ((ret = pthread_create(&f->thread, NULL, input_thread, f))) {
            ALOGE( "pthread_create failed: %s. Try to increase `ulimit -v` or decrease `ulimit -s`.\n", strerror(ret));
            av_thread_message_queue_free(&f->in_thread_queue);
            return AVERROR(ret);
        }
    }
...
static int process_input(int file_index)
{
    ...
    if (exit_on_error && (pkt.flags & AV_PKT_FLAG_CORRUPT)) {
        ALOGE( "%s: corrupt input packet in stream %d\n", is->filename, pkt.stream_index);
        exit_program(1);
    }

    if (debug_ts) {
        ALOGI( "demuxer -> ist_index:%d type:%s "
    ...
    if(!ist->wrap_correction_done && is->start_time != AV_NOPTS_VALUE && ist->st->pts_wrap_bits < 64){
        ...
        if (   ist->next_dts == AV_NOPTS_VALUE
            ...
            if (new_start_time > is->start_time) {
                ALOGI( "Correcting start time by %"PRId64"\n", new_start_time - is->start_time);
    ...
    if ((ist->dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO ||
        ...
        if (is->iformat->flags & AVFMT_TS_DISCONT) {
            ...
            if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                ...
                ALOGI( "DTS %"PRId64", next:%"PRId64" st:%d invalid dropping\n", pkt.dts, ist->next_dts, pkt.stream_index);
                pkt.dts = AV_NOPTS_VALUE;
            }
            if (pkt.pts != AV_NOPTS_VALUE){
                ...
                if ( delta < -1LL*dts_error_threshold*AV_TIME_BASE ||
                     delta >  1LL*dts_error_threshold*AV_TIME_BASE) {
                    ALOGI( "PTS %"PRId64", next:%"PRId64" invalid dropping st:%d\n", pkt.pts, ist->next_dts, pkt.stream_index);
                    pkt.pts = AV_NOPTS_VALUE;
                }
            }
        }
    ...
    if (debug_ts) {
        ALOGI( "demuxer+ffmpeg -> ist_index:%d type:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
               ifile->ist_index + pkt.stream_index, av_get_media_type_string(ist->dec_ctx->codec_type),
               av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
               av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
               av_ts2str(input_files[ist->file_index]->ts_offset),
...
static int transcode_step(void)
{
    ...
    if (!ost) {
        ...
        ALOGI( "No more inputs to read from, finishing.\n");
        return AVERROR_EOF;
    }
...
static int transcode(void)
{
    ...
    if (stdin_interaction) {
        ALOGI( "Press [q] to stop, [?] for help\n");
    }
    ...
    while (!received_sigterm) {
        ...
        if (!need_output()) {
            ALOGI( "No more output streams to write to, finishing.\n");
            break;
        }
    ...
    for (i = 0; i < nb_output_files; i++) {
        ...
        if ((ret = av_write_trailer(os)) < 0) {
            ALOGE( "Error writing trailer of %s: %s", os->filename, av_err2str(ret));
            if (exit_on_error)
                exit_program(1);
        }
    }
...
int main_ffmpeg(int argc, char **argv)
{
    ...
    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        ALOGI( "Use -h to get full help or, even better, run 'man %s'\n", program_name);
        exit_program(1);
    }

    /* file converter / grab */
    if (nb_output_files <= 0) {
        ALOGE( "At least one output file must be specified\n");
        exit_program(1);
    }
...
//     }
    ...
    ti = getutime() - ti;
    if (do_benchmark) {
        ALOGI( "bench: utime=%0.3fs\n", ti / 1000000.0);
    }
    ALOGI( "%"PRIu64" frames successfully decoded, %"PRIu64" decoding errors\n",
...
JNIEXPORT jint JNICALL Java_com_ytx_ican_ytxplayer_MainActivity2_setCmd
    ...
    char *cmds;
    char info[40000] = { 0 };
    const char *play_cmd = (*env)->GetStringUTFChars(env,cmd,NULL);
    ALOGI("cmd=%s",play_cmd);

    for(i=0;play_cmd[i]!='\0';i++){
        if(play_cmd[i]==' '){
            argc++;
        }
    }
    ...
    int len = strlen(play_cmd);
    cmds = malloc(sizeof(char *)*(len+1));


    strcpy(cmds,play_cmd);
    cmds[len]='\0';
    ALOGI("argc %d cmds=%s\n",argc,cmds);
    argv[j]=cmds;
    for(i=0;cmds[i]!='\0';i++){
