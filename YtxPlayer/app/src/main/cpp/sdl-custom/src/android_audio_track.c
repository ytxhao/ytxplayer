...
#define LOG_NDEBUG 0
#define TAG "YTX-ANDROID-AUDIO-TRACK-JNI"
#include "ytxplayer/ALog-priv.h"

#include "../include/AudioTrack.util.h"
#include "../include/android_audio_track.h"
#include <assert.h>
...
SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_spec(JNIEnv *env, SDL_Android_AudioTrack_Spec *spec)
{
    ...
    init_volume = YTXMIN(init_volume, atrack->max_volume);
    init_volume = YTXMAX(init_volume, atrack->min_volume);
...
int SDL_Android_AudioTrack_reserve_byte_buffer(JNIEnv *env, SDL_Android_AudioTrack *atrack, int size_in_byte)
{
    ...
    int capacity = YTXMAX(size_in_byte, atrack->min_buffer_size);
